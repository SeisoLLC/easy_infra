...
update-functions:
	...
	@docker run --rm -v $$(pwd):/usr/src/app -w /usr/src/app python:3.9 /bin/bash -c "python3 -m pip install --upgrade pip && pip install --user -r requirements.txt && pwd && ./update_bash_env.py --config-file easy_infra.yml --output functions --template-file functions.j2"
	@echo "Done!"
...
update-requirements: requirements-to-freeze.txt
	...
	@docker run --rm -v $$(pwd):/usr/src/app/ python:3.9 /bin/bash -c "python3 -m pip install --upgrade pip && pip3 install -r /usr/src/app/requirements-to-freeze.txt && pip3 freeze > /usr/src/app/requirements.txt"
	@echo "Done!"
