	...
	@i=0; for element in $(FUNCTION_GENERATOR); do \
		if (( i % 4 == 0 )) ; then \
		  echo "# DO NOT EDIT THIS FILE BY HAND -- YOUR CHANGES WILL BE OVERWRITTEN" >> functions; \
			...
			docker run --rm -v $$(pwd):/tmp/ easy_infra:latest /bin/bash -c "sed -i 's/%FUNCTION_NAME%/$${element}/g' /tmp/functions"; \
		elif (( i % 4 == 1 )) ; then \
			docker run --rm -v $$(pwd):/tmp/ easy_infra:latest /bin/bash -c "sed -i 's/%SECURITY_TOOL%/$${element}/g' /tmp/functions"; \
		elif (( i % 4 == 2 )) ; then \
			docker run --rm -v $$(pwd):/tmp/ easy_infra:latest /bin/bash -c "sed -i 's/%SECURITY_TOOL_ARGS%/$${element}/g' /tmp/functions"; \
		elif (( i % 4 == 3 )) ; then \
			docker run --rm -v $$(pwd):/tmp/ easy_infra:latest /bin/bash -c "sed -i 's/%SECURITY_TOOL_ACTION%/$${element}/g' /tmp/functions"; \
		fi; \
		i=$$((i+1)); \
	done
	@echo "Done!"
